...
import { getLegajosData } from '@/lib/areas/get-legajos-data'
import { LegajosClient } from '@/components/areas/legajos-client'
...
export default async function LegajosPage() {
  ...
  if (!perms.includes('area.admin.view') && !perms.includes('area.admin.manage')) {
    redirect('/areas')
  }

  const data = await getLegajosData()

  return (
    <div className="max-w-[1400px]">
      <div style={{ marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
        <Link
          href="/areas/administracion"
      ...
      <AreaHero areaKey="administracion" jefeArea={null} />

      <LegajosClient data={data} />
    </div>
  )
}
